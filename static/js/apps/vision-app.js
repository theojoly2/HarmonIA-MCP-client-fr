...
class VisionApp extends AppBase {
    ...
    _extractClassNames() {
        ...
        // Extract class names from SVG nodes. Nodes generated by semantic_model use
        // data-element-id and the label text inside a tspan with class "node-label".
        const parser = new DOMParser();
        const doc = parser.parseFromString(this.svgText, 'image/svg+xml');
        const names = new Set();
        doc.querySelectorAll('g.node').forEach((g) => {
            const label = g.querySelector('text.node-label tspan');
            if (label && label.textContent) names.add(label.textContent.trim());
        });
        // Fallback: data-element-id may hold the URI/local-name for cases without text.
        if (names.size === 0) {
            doc.querySelectorAll('[data-element-id]').forEach((el) => {
                const id = el.getAttribute('data-element-id');
                if (id) names.add(id.split(':').pop());
            });
        }
