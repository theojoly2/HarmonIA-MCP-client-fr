...
<body class="bg-white text-gray-900 font-sans antialiased selection:bg-gray-300">
    ...
    <script>
        ...
        function showVisionHome() {
            ...
            const existingWrapper = document.getElementById('vision-page-wrapper');
            const existingViewer = document.getElementById('svg-viewer');

            // Already on home: nothing to do.
            if (existingWrapper && !existingViewer) {
                showPane('vision');
                return;
            }

            // If we are in import/view mode, animate the existing wrapper down
            // and restore the import area, then remove the SVG viewer.
            if (existingWrapper && existingViewer) {
                const importContainer = document.getElementById('vision-import-container');

                // Reveal import container first (no transition, so it can expand while wrapper moves down)
                if (importContainer) {
                ...
                existingWrapper.classList.remove('vision-top');
                existingWrapper.style.justifyContent = 'flex-start';
                existingWrapper.style.paddingTop = '96px';
                existingWrapper.style.paddingBottom = '0';

                // Fade out and remove the viewer
                existingViewer.style.transition = 'opacity 0.35s ease';
                existingViewer.style.opacity = '0';
                setTimeout(() => {
            ...
            // Fallback: rebuild the whole Vision home from scratch.
            content.innerHTML = `
                <div id="vision-page-wrapper" class="px-4 sm:px-6 flex flex-col items-center text-center flex-shrink-0 z-20 bg-white transition-all duration-500 ease-out">
                    <h1 class="font-bold tracking-tight text-center text-black mb-2 mt-2">
                        <button type="button" class="interactive-title bg-transparent border-0 p-0" onclick="showVisionHome();" title="Retour à l'accueil Vision">
        ...
        function showPane(pane) {
            ...
            if (pane === 'search') {
                ...
                // Recenter after the pane transition has settled.
                setTimeout(() => {
                    applyVisionCentering();
                }, 120);
            } else if (pane === 'split') {
        ...
        function applyVisionCentering() {
            ...
            if (visionWrapper.classList.contains('vision-top')) {
                ...
                // Fixed top padding for a stable home page position regardless of viewport.
                visionWrapper.style.paddingTop = '96px';
                visionWrapper.style.paddingBottom = '0';
            }
        }
